<template>
  <section class="page">
    <div class="page__header">
      <div>
        <h1 class="page__title">实时检测</h1>
        <p class="page__desc">当前阶段可直接联调视频流、状态统计和异常截图。</p>
      </div>
      ...
      <div class="page__header-actions">
        <label class="field field--light">
          ...
          <select v-model="selectedVideoSource">
            <option v-for="source in availableVideoSources" :key="source.value" :value="source.value">
              {{ source.label }}
            </option>
          </select>
        </label>
            ...
            <div class="video-overlay__actions">
              ...
              >
                ...
                <svg v-else viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  />
                  <path d="M10 10h4v4h-4z" fill="none" />
                </svg>
              </button>
            </div>
          </div>
      ...
      <aside class="sidebar">
        ...
        <section class="panel">
          ...
          <div class="system-info">
            ...
            <div>姿态分析：{{ settings.enable_pose_analysis ? "开启" : "关闭" }}</div>
            <div>异常截图：{{ settings.save_alert_snapshots ? "保存" : "不保存" }}</div>
...
const availableVideoSources = computed(() => {
  ...
  for (const source of settings.recent_video_sources ?? []) {
    if (!source?.value) {
      continue;
    }
    options.set(source.value, source);
  }
  if (settings.network_video_source?.trim()) {
    options.set(settings.network_video_source, {
      label: settings.network_video_source,
      value: settings.network_video_source,
    });
  }
...
async function bootstrap() {
  ...
  const currentSelection = selectedVideoSource.value;
  const knownValues = new Set(
    (settingsData.recent_video_sources ?? []).map((item) => item.value).filter(Boolean)
  );
  if (settingsData.network_video_source) {
    knownValues.add(settingsData.network_video_source);
  }
  if (!currentSelection || !knownValues.has(currentSelection)) {
    selectedVideoSource.value =
      sessionData.video_source || settingsData.network_video_source || settingsData.local_camera_source || "0";
  }
}
...
async function handleStart() {
  ...
  try {
    ...
    const response = await startSession({
      ...
      enable_pose_analysis: settings.enable_pose_analysis,
      save_alert_snapshots: settings.save_alert_snapshots,
    });
